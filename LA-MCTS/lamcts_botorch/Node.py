...
class Node:
    ...
    def __init__(
        ...
        self.classifier = Classifier(
            [], self.dims, kernel_type, gamma_type, use_botorch
        )
    ...
    def __str__(self):
        ...
        name += self.pad_str_to_8chars(
            " val:{0:.4f}   ".format(round(self.get_xbar(), 3)), 20
        )
        name += self.pad_str_to_8chars(
            " uct:{0:.4f}   ".format(round(self.get_uct(), 3)), 20
        )
    ...
    def get_uct(self, Cp=10):
        ...
        if self.n == 0:
            return float("inf")
        return self.x_bar + 2 * Cp * math.sqrt(
