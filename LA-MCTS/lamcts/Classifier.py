...
class Classifier:
    ...
    def fit_gpr(self, X, fX):
        ...
        # noise = 0.1**2
        # kernel = MaternKernel(nu=2.5, lengthscale_prior=1.0)
        # self.gpr = SingleTaskGP(X_tensor, fX_tensor, fX_var, covar_module=kernel)
    ...
    def propose_rand_samples_probe(self, nums_samples, path, lb, ub):
        ...
        for axis in range(0, axes):
            ...
            while ratio >= 0.9:
                ...
                cands_ = (
                    sobol.draw(10000).to(dtype=torch.float64).cpu().detach().numpy()
                )
